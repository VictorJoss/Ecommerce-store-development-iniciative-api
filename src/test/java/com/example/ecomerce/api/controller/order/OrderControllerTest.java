...
public class OrderControllerTest {
    ...
    private void testAuthenticatedListBelongsToUser(String username) throws Exception{
        mvc.perform(get("/api/order")).andExpect(status().is(HttpStatus.OK.value()))
                .andExpect(result -> {
                    String json = result.getResponse().getContentAsString();
                    List<WebOrder> orders = new ObjectMapper().readValue(json, new TypeReference<List<WebOrder>>() {});
                    for (WebOrder order : orders){
                        Assertions.assertEquals(username, order.getUser().getUsername(), "Order List should only be orders beloging to the user");
                    }
                });
    }
    ...
    public void testUnauthenticatedOrderList() throws Exception{
        mvc.perform(get("/api/order")).andExpect(status().is(HttpStatus.FORBIDDEN.value()));
    }
}
